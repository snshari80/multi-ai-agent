...
async def author_agent_node(state:AgentState):
    ...
    if draft and not draft.lower().startswith(("i couldn't", "the query failed")):
        prompt = ChatPromptTemplate.from_template(_SYNTHESIS_PROMPT)
        chain = prompt | get_openaillm()
        polishd = (
    ...
    if pii_found:
        existing_flags += [f"Pii_masked:{t}" for t in pii_found]
    ...
    return {
        ...
        "latency_ms":{**state.get("latency_ms",{}),"author":f"{latency} ms"}
    }
